...
```{r}
plot(R_sq ~ Year, data = pctSalary_TimeAnalysis, xlab = "Year", ylab = "R-Squared", main = "R-Squared values for SLR of Win Percentage and Team Payroll Percentage", pch  = 1, cex  = 1, col  = "orange")
```
...
```{r}
(quad_loocv_rmse = get_loocv_rmse(mlb_lm_fore_bic))
```
...
Note that the LOOCV RMSE of the Quadratic Model is `r quad_loocv_rmse`, which is less than the main section model one, `r main_loocv_rmse`.  This means that the Quadratic Model is better in predicting win percentage.  However, in viewing the plots of the quadratic terms below, we see several influential observations that appear to be the cause of a somewhat false quadratic relationship for the variables `BB`, `H`, and `attendance`. We therefore decided to keep the main model as it is simpler and more consistent with conventional wisdom.

```{r }
library(ggplot2)
ggplot(data = mlb_lm_year_filtered_data, aes(x = mlb_lm_year_filtered_data$BB, y = mlb_lm_year_filtered_data$WinPct)) +
  stat_smooth(method = "lm", se = FALSE, color = "green", formula = y ~ x) +
  stat_smooth(method = "lm", se = FALSE, color = "blue", formula = y ~ x + I(x ^ 2)) +
  geom_point(colour = "black", size = 3)
ggplot(data = mlb_lm_year_filtered_data, aes(x = mlb_lm_year_filtered_data$attendance, y = mlb_lm_year_filtered_data$WinPct)) +
  stat_smooth(method = "lm", se = FALSE, color = "green", formula = y ~ x) +
  stat_smooth(method = "lm", se = FALSE, color = "blue", formula = y ~ x + I(x ^ 2)) +
  geom_point(colour = "black", size = 3)
ggplot(data = mlb_lm_year_filtered_data, aes(x = mlb_lm_year_filtered_data$H, y = mlb_lm_year_filtered_data$WinPct)) +
  stat_smooth(method = "lm", se = FALSE, color = "green", formula = y ~ x) +
  stat_smooth(method = "lm", se = FALSE, color = "blue", formula = y ~ x + I(x ^ 2)) +
  geom_point(colour = "black", size = 3)
```
