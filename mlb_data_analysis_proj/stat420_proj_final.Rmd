...
Recall that the model we choose in the main section is
```{r}
mlb_lm_both_bic
```
...
Note that the LOOCV RMSE of the Quadratic Model is `r quad_loocv_rmse`, which is less than the main section model one, `r main_loocv_rmse`.  This means that the Quadratic Model is better in predicting win percentage.  However, we kept the main model as it is much simpler.

#### Compare models

```{r}
bigger_model = lm(WinPct ~ runDifferential + SV + I(BB^2) + BB + CG + I(attendance^2) + pctSalary, data = mlb_lm_year_filtered_data)
